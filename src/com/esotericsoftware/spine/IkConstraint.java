...
public class IkConstraint {
	...
	/** Copy constructor. */
	public IkConstraint (IkConstraint ikConstraint, ArrayList<Bone> bones, Bone target) {
	...
	public void apply () {
		...
		switch (bones.size()) {
		case 1:
			apply(bones.get(0), target.worldX, target.worldY, mix);
			break;
		case 2:
			apply(bones.get(0), bones.get(1), target.worldX, target.worldY, bendDirection, mix);
			break;
		}
	}
	...
	static public void apply (Bone bone, float targetX, float targetY, float alpha) {
		...
		// Replaced MathUtils.radDeg with 180 / Math.PI
		float rotationIK = (float)Math.atan2(targetY - bone.worldY, targetX - bone.worldX) * (float) (180f / Math.PI) - parentRotation;
	...
	static public void apply (Bone parent, Bone child, float targetX, float targetY, int bendDirection, float alpha) {
		...
		if (parentParent != null) {
			...
			parentParent.worldToLocal(new Vector2f(targetX, targetY));
		...
		if (child.parent == parent)
			...
			parent.worldToLocal(child.parent.localToWorld(new Vector2f(child.x, child.y)));
		...
		if (cosDenom < 0.0001f) {
			// Replaced MathUtils.radDeg with 180 / Math.PI
			child.rotationIK = childRotation + ((float)Math.atan2(targetY, targetX) * (float) (180f / Math.PI) - parentRotation - childRotation)
				* alpha;
			return;
		}
		float cos = MathUtils.clamp((targetX * targetX + targetY * targetY - len1 * len1 - len2 * len2) / cosDenom, -1f, 1f);
		float childAngle = (float)Math.acos(cos) * bendDirection;
		float adjacent = len1 + len2 * cos, opposite = (float) (len2 * Math.sin(childAngle));
		float parentAngle = (float)Math.atan2(targetY * adjacent - targetX * opposite, targetX * adjacent + targetY * opposite);
		float rotation = (parentAngle - offset) * (float) (180f / Math.PI) - parentRotation;
		if (rotation > 180)
			rotation -= 360;
		else if (rotation < -180) //
			rotation += 360;
		parent.rotationIK = parentRotation + rotation * alpha;
		rotation = (childAngle + offset) * (float) (180f / Math.PI) - childRotation;
		if (rotation > 180)
			rotation -= 360;
		else if (rotation < -180) //
			rotation += 360;
